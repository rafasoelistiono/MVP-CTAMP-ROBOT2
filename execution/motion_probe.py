...
class MotionProbe:
    ...
    def probe_pick_feasibility(
        ...
        if not self._backend_available:
            return self._geometric_pick_probe(world, obj)
        return self._backend_pick_probe(world, object_id, obj)
    ...
    def probe_place_feasibility(
        ...
        if not self._backend_available:
            return self._geometric_place_probe(world, slot_pos)
        return self._backend_place_probe(world, object_id, slot_pos)
    ...
    def _geometric_pick_probe(
        ...
        near_obs = any(
            math.dist(obj.pose[:2], obs.pose[:2]) < obs.radius + 0.08
    ...
    def _geometric_place_probe(
        ...
        in_reach = world.robot_reach_min <= dist <= world.robot_reach_max
        near_obs = any(
            math.dist(slot_pos[:2], obs.pose[:2]) < obs.radius + 0.08
            for obs in world.obstacles
        )
        feasible = in_reach and not near_obs
        return ProbeResult(
    ...
    def _backend_pick_probe(
        ...
        try:
            import math
            from backends.mujoco import runtime as mujoco_runtime
            ...
            ik_result = mujoco_runtime._solve_ik_to(grasp_pos)
            if ik_result is None or ik_result[0] is None:
                ...
                return ProbeResult(
                    ...
                    failure_reason="ik_solve_failed",
                )
            goal_q = ik_result[0]
            planner = mujoco_runtime._get_ompl_planner()
            ompl_ok = True
            if planner is not None:
                ompl_ok = planner.is_state_valid_q(goal_q)
            planning_time = time.perf_counter() - start
            return ProbeResult(
                feasible=ompl_ok,
                ik_success=True,
                ompl_success=ompl_ok,
                planning_time=round(planning_time, 4),
                estimated_path_length=dist,
                min_clearance=0.10,
                collision_count=0,
                failure_reason=None if ompl_ok else "ompl_state_invalid",
            )
    ...
    def _backend_place_probe(
        ...
        try:
            ...
            dist = math.dist(slot_pos[:2], world.robot_base_xy)
            ik_result = mujoco_runtime._solve_ik_to(slot_pos)
            if ik_result is None or ik_result[0] is None:
                planning_time = time.perf_counter() - start
                return ProbeResult(
                    feasible=False,
